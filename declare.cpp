...
void lwd::stprint(SimpleType s) {
	...
	bool function_head::semanticCheck(function_decl *root, int scope) {
		...
		return child2==NULL?true:child2->semanticCheck(child1, gtable.insertScope(root, scope));
	}
	...
	bool proc_stmt::semanticCheck(int scope) {
		...
		case 5:
			return child1.choice5->semanticCheck(scope);
		default: return false; // error
		}
	}
	...
	bool goto_stmt::semanticCheck(int scope) {
		bool temp1 = gtable.existLabel(child1);
		if(!temp1)
			fprintf(stderr, "Label %d used but never defined.\n", child1);
		return temp1;
	}
