...
std::string tString(Type type, SimpleType stype) {
	...
	case aarray_type: //not implemented
	case rrecord_type:
...
bool typeEqual(identifier *id, expression *exp) {
	...
	return tEqual(id->type==ffunction?ssimple_type:id->type, id->stype, ssimple_type, exp->value.stype);//support simple type only now
}
